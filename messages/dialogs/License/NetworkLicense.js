...
module.exports.GetNetworkLicense = [
  ...
  (session, results, next) => {
    ...
    let herocards = new builder.Message(session).attachments([
  ...
  (session, results, next) => {
    let herocards = new builder.Message(session).attachments([
      new builder.HeroCard(session)
        .text(
          `Start the installation by launching “setup.exe” from the downloaded ISO image.`
        )
        .images([
  ...
  (session, results, next) => {
    let herocards = new builder.Message(session).attachments([
      new builder.HeroCard(session)
        .text(
          `Make sure you select “Use a File Installation Key”. Then click “Next”.`
        )
        .images([
          builder.CardImage.create(
            session,
  ...
  (session, results, next) => {
    let herocards = new builder.Message(session).attachments([
      new builder.HeroCard(session)
        .text(
          `Enter the following File Installation Key (make sure to avoid any leading and/or trailing spaces). Then click “Next”.19811-63381-39632-64689-45816-28031-02371-11344-23344-55051-54611-49353-44636-41025-10294-22461-63683-46964-30928-12461-48246-33632-02160`
        )
        .images([
          builder.CardImage.create(
            session,
  ...
  (session, results, next) => {
    let herocards = new builder.Message(session).attachments([
      new builder.HeroCard(session)
        .text(
          `Review the installation folder and change if necessary. Then click “Next”.`
        )
        .images([
          builder.CardImage.create(
            session,
  ...
  (session, results, next) => {
    let herocards = new builder.Message(session).attachments([
      new builder.HeroCard(session)
        .text(
          `Make sure the License Manager component is unselected. Click “Next”.`
        )
        .images([
          builder.CardImage.create(
            session,
  ...
  (session, results, next) => {
    let herocards = new builder.Message(session).attachments([
      new builder.HeroCard(session)
        .text(
          `select the license.dat file you downloaded along with the MATLAB binaries. Then click “Next”.`
        )
        .images([
          builder.CardImage.create(
            session,
  ...
  (session, results, next) => {
    let herocards = new builder.Message(session).attachments([
      new builder.HeroCard(session)
        .text(
          `Review the installation options and click “Next” in order to start the installation.`
        )
        .images([
          builder.CardImage.create(
            session,
  ...
  (session, results, next) => {
    let herocards = new builder.Message(session)
      ...
      .attachments([
        new builder.HeroCard(session)
          .text(
            `A progress indicator will be displayed. The installation of MATLAB can take 20 to 40 minutes depending on the speed of your computer`
          )
          .images([
            builder.CardImage.create(
              session,
        ...
        new builder.HeroCard(session)
          .text(
            `Note that You will be asked to insert DVD2. If you are mounting an ISO image using virtual clone drive under Win7 or the built-in Win 8.1 / 10 function. unmount it first. This can be done typically by right-clicking on the virtual DVD drive and selecting unmount.`
          )
          .images([
            builder.CardImage.create(
              session,
              `http://buzzbot-it.gatech.edu/wp-content/uploads/sites/677/2018/03/Step112.png`
            )
          ])
      ]);
    session.send(herocards);
  ...
  (session, results, next) => {
    let herocards = new builder.Message(session).attachments([
      new builder.HeroCard(session)
        .text(
          `review the additional configuration steps (if applicable). Then select “Next”.`
        )
        .images([
          builder.CardImage.create(
            session,
