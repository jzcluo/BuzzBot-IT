...
module.exports.ActivateLicense = [
  (session, args) => {
    ...
    let suggestedActions = SuggestedActionsMessage(session, "It looks like you are trying to activate your individual license. Is that correct?", choiceList);
  ...
  (session, results, next) => {
    session.send("The steps for activating individual license is not yet implemented");
    next();
  },
  (session, results, next) => {
    let choiceList = ["Yes", "No"];
