...
module.exports.DeactivateLicense = [
  (session, args) => {
    ...
    let suggestedActions = SuggestedActionsMessage(
      session,
      'It looks like you are trying to deactivate your individual license. Is that correct?',
      choiceList
    );
  ...
  (session, results, next) => {
    session.send(
      'The steps for deactivating individual license is not yet implemented'
    );
    next();
  },
  (session, results, next) => {
