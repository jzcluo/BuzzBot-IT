...
module.exports.ReactivateExpiringLicense = [
  (session, args) => {
    ...
    let suggestedActions = SuggestedActionsMessage(
      session,
      "It looks like you are trying to reactivate your expiring individual license. Is that correct?",
      choiceList
    );
  ...
  (session, results, next) => {
    session.send(
      "The steps for reactivating expiring individual license is not yet implemented"
    );
    next();
  },
  (session, results, next) => {
