...
module.exports.ReactivateExpiredLicense = [
  (session, args) => {
    ...
    let suggestedActions = SuggestedActionsMessage(
      session,
      'It looks like you are trying to reactivate your expired individual license. Is that correct?',
      choiceList
    );
  ...
  (session, results, next) => {
    session.send(
      'The steps for reactivating expired individual license is not yet implemented'
    );
    next();
  },
  (session, results, next) => {
