...
module.exports.InstallMatlabHelp_Windows = [
  (session, args, next) => {
    ...
    let waterfallStepIndex = session.dialogStack()[args.dialogIndex].state[
      "BotBuilder.Data.WaterfallStep"
    ];
    ...
    session.send(
      `So you are stuck on ${
        waterfallStepIndex - 1 < 9
          ? waterfallSteps[waterfallStepIndex - 1]
          : waterfallSteps[8]
      } of the installation. Let's help you with that.`
    );
    ...
    session.send(
      waterfallStepIndex - 1 < 9
        ? helpMessages[waterfallStepIndex - 1]
        : helpMessages[8]
    );
    next();
  },
